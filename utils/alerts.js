...
const STEP_BLOCKS = 10;
const INTERVAL_MS = 20000;
let nftCache = { totalMinted: 0, timestamp: 0 };

function safeBlock(bn) {
  return bn < 0n ? 0n : bn;
}

// Função com retry para eventos
...
// Função para contar NFTs mintados
async function getTotalMinted() {
  ...
  for (let from = 0; from <= latestBlock; from += STEP_BLOCKS) {
    const to = Math.min(from + STEP_BLOCKS, latestBlock);
    try {
