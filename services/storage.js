...
function defaultDb(){
  return {
    users: {}, // telegramId -> { wallet, username, totalToday, totalAllTime, lastDropDay }
...
function setUser(telegramId, obj){
  const db = read();
  db.users[String(telegramId)] = Object.assign(db.users[String(telegramId)] || {}, obj);
  write(db);
...
module.exports = { ensure, read, write, getUser, setUser, addWithdrawal, listWithdrawals, popWithdrawal, updateConfig, defaultDb };
